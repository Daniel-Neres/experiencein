...
def convidar(request, *args, **kwargs):
    ...
    if perfil_a_convidar != perfil_logado:
        ...
        return response.Response({'message': f'Convite enviado com sucesso para {perfil_a_convidar.email}.'}, status=status.HTTP_201_CREATED)
...
def aceitar(request, *args, **kwargs):
    ...
    convite.aceitar()
    return response.Response({'message': 'Convite aceito com sucesso.'},status=status.HTTP_201_CREATED)
